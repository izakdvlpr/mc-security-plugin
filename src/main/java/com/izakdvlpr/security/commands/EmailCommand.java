...
public class EmailCommand implements CommandExecutor {
  ...
  public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
    ...
    if (!(args.length == 1)) {
      sender.sendMessage("§ePara cadastrar seu email, use: /email <email>");
    ...
    System.out.print(ZKSecurityMain.getInstance().getConfig().getString("smtp.driver.host"));

    mailerService.sendEmail("izakdvlpr@gmail.com", email,"[Servidor] Verificação de E-mail", "Olá usuário este é seu código " + code + ", use o comando /verify " + code + " para validar este código e verificar seu perfil como seguro.");
